...
- `triageSource: "code_deterministic"` + `needsModelReview: false` → the code already classified this reliably with a known pattern. You may accept it directly, use its `category` / `recoveryAction` / `sameProblemKey` as-is, and move on. No re-analysis is required.
...
- **For overlay-policy ambiguity** (ui_overlay_policy sensitive candidates): re-read the captured UI hierarchy / screenshots. Decide whether each ambiguous control is a safe-dismiss (e.g. close-X, cancel, skip, later) or requires user authorization. If you authorize a dismiss, record the authorization in `evaluation.json.uiUnblock.authorizedLabels[]` so the next Generator/probe-flow run can use it; if not, record it as `permission_blocked` in failedChecks.
