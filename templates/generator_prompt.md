...
- **Prior-round model-review signals (MANDATORY on retry/repair turns):** when you read `evaluation.json`, `runtime-state.json`, or the evaluator-context pack, scan every structured entry for the `needsModelReview` field.
  ...
  - `needsModelReview: false` + `triageSource: "code_deterministic"` → the code classified this with a known pattern. You may act directly on its `category` / `recoveryAction` / `sameProblemKey`. If the recovery action is a command (e.g. run `pod install`, run `automind ios-signing-preflight`, run `<AUTOMIND_CLI> dependency-check <task-code>`), execute it before retrying the failing build/test command.
  - `needsModelReview: false` + `triageSource: "model_reviewed"` → a previous Evaluator iteration already re-examined and classified this. Trust its `category` / `recoveryAction` / `specificErrors`.
